...
\lstset{
    ...
    keywords=[20]{-recorder,-shell-escape,--latex,-s,-o,--arara,--norun,--luatex,--latexmk,--svg,--png,--nopdf},%
...
\item If script is call \emph{without} \cmdopt[n]{noprew} options, the following lines
...
The different parts of the file read in memory are joined  and save in a temporary file \sysfile{test-fig-1981.tex}
in \sysdir{./}. This file will contain all the environments for extraction between \lstinline|\begin{preview}|\ldots\lstinline|\end{preview}| along with the rest of the document.

\item If script is call \emph{with} \cmdopt[n]{noprew} options, the \lstinline|\begin{preview}|\ldots\lstinline|\end{preview}|
...
in a temporary file \sysfile{test-fig-1981.tex} in \sysdir{./}.
...
\end{examplecmd}
generating the file \sysfile{test-fig-1981.pdf} with all code extracted, move \sysfile{test-fig-1981.pdf}
...
Write a \fext{log} file with all process information.
...
passed need \textcolor{optcolor}{\textbf{\texttt{-\/-}}} at the end. The environments \env{document}
...
an \meta[cf=optcolor]{output file} \sysfile{file-out.tex} with all extracted environments
...
an \meta[cf=optcolor]{output file} \sysfile{file-out.tex} with all exatracted environments
...
an output file \meta[cf=optcolor]{file-out.tex} with all \env{picture} environments
converted to \ics{includegraphics}, a single file \sysfile{file-in-fig-all.ltx} with only environments \env{picture} extracted
using \prgname*{lualatex} and \pkgname{preview} package for process \meta[cf=optcolor]{file-in.tex} and
\scriptname{latexmk} for \emph{full} process  \meta[cf=optcolor]{file-out.tex}.
