...
\hypersetup{
  ...
  pdfcopyright       = {\textcopyright 2013 - 2024 by Pablo González Luengo},
...
\NewDocumentCommand{\fext}{m}
  {
    ...
    \SortIndex{file ~ extension}{File ~ extensions >\small\texttt{.#1}}
  }
...
\NewDocumentCommand{\myenv}{m}
  {
    ...
    \end{minipage}%
    \SortIndex{Environment}{Environments support by default>\small\texttt{#1}}%
  }
...
\title{%
    ...
    \author{%
    \large%
    \raisebox{-1pt}{\textcopyright}{}2013--2024 by Pablo González L%
...
invoked from \emph{standard} \prgname{cmd}, it is recommended to use a
...
In the fifth step, the script apply the option \cmdopt{clean}, remove all content between \lstinline|%<*remove> ... %</remove>|
...
\DescribeTE*{noltximg}
All content between \lstinline|%<*noltximg> ... %</noltximg>| are ignored and no extract.
The tags can \emph{not} be nested and should be at the beginning of the line and in separate lines.
...
\DescribeTE*{remove}
All content between \lstinline|%<*remove> ... %</remove>| are deleted in the \meta[cf=optcolor]{output file}.
The tags can \emph{not} be nested and should be at the beginning of the line and in separate lines.
...
The content between \lstinline|%<*remove> ... %</remove>| are remove from output file
...
and generate \fext{dvi} or \fext{xdv} files, then running:
...
\item Execute the script using \cmdopt{norun}{}, move to \sysdir{./images} and generate  \fext{dvi} or \fext{xdv} file, then running:
...
\section{Example using \texttt{latexmk}}
...
\item [v1.8 (ctan), 2020-08-18] \leavevmode\vspace{-\baselineskip}
    \begin{itemize}
        ...
        \item Re-write source code according to \texttt{Perl} v5.3x.
...
\item [v1.3 (devp), 2016-08-14] \leavevmode\vspace{-\baselineskip}
    \begin{itemize}
        ...
        \item Support \texttt{minted} and \texttt{tcolorbox} package.
